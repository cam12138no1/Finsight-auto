      ...
      <div className="w-full max-w-2xl mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-900">选择已下载财报</h3>
              <p className="text-xs text-slate-500">从数据库中选择 SEC EDGAR 已下载的财报进行 AI 分析</p>
            </div>
          </div>
        ...
        <div className="px-6 py-3 border-b bg-slate-50 flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="搜索公司..." value={searchQuery}
              ...
              className="w-full h-8 pl-9 pr-3 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
          </div>
          <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)}
            className="h-8 px-2 rounded-lg border border-slate-200 text-xs cursor-pointer">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-500">暂无已下载财报</p>
              <p className="text-xs text-slate-400 mt-1">请先到下载中心创建下载任务</p>
            </div>
          ) : (
                  ...
                  <div key={ticker} className="space-y-1">
                    <div className="flex items-center gap-2 px-1 mb-1">
                      <div className={`w-6 h-6 rounded flex items-center justify-center text-[9px] font-bold ${
                        isApp ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'
                      }`}>{ticker.slice(0, 2)}</div>
                      <span className="text-sm font-semibold text-slate-700">{first.company_name}</span>
                    ...
                    {companyFilings.map(f => (
                      <button key={f.id} onClick={() => setSelectedId(f.id === selectedId ? null : f.id)}
                        ...
                        <FileText className={`h-4 w-4 shrink-0 ${selectedId === f.id ? 'text-blue-600' : 'text-slate-400'}`} />
